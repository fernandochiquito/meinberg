<?php 
if (isset($_POST['txtdest']))
{
	$destino = $_POST['txtdest'];
	$assunto = $_POST['txtass'];
	$mensagem = $_POST['txtmsg'];
	if (PATH_SEPARATOR ==":") {
		$quebra = "\r\n";
	} else {
		$quebra = "\n";
	}
	$headers = "MIME-Version: 1.1".$quebra;
	$headers .= "Content-type: text/plain; charset=iso-8859-1".$quebra;
	$headers .= "From: paulo@meinberg.biz".$quebra; //E-mail do remetente
	$headers .= "Return-Path: paulo@meinberg.biz".$quebra; //E-mail do remetente
	mail($destino, $assunto, $mensagem, $headers, "-r". "paulo@meinberg.biz");
	print "Mensagem enviada com sucesso!";
}else{ ?>

<html lang="pt">

<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0" />
  <link rel="stylesheet" type="text/css" href="/css/normalize.css/normalize.css" />
  <link rel="stylesheet" type="text/css" href="css/styles.css" />
  <link
    href="https://fonts.googleapis.com/css2?family=Lato:ital,wght@0,100;0,300;0,400;0,700;0,900;1,100;1,300;1,400;1,700;1,900&display=swap"
    rel="stylesheet" />
  <title>Meinberg Administração e Corretagem de Seguros</title>
  <meta name="keywords"
    content="Seguro, seguro de vida, Seguro Residência, Seguro Empresarial, Seguro Saúde, Seguro de Automóvel, Previdência Privada, seguro Fiança, " />
  <meta name="description"
    content="Meinberg Administração e Corretagem de Seguros com foco em Seguros de Vida,  Previdência Privada, Soluções para Locação (Fiança / Capitalização / Imobiliária), Riscos de Engenharia (Energia Solar / Placas Fotovoltaicas), Seguro Residência, Seguro Empresarial, Seguro Saúde, Seguro de Automóvel" />
</head>

<body>
  <header class="site-header">
    <div class="wrapper">
      <div class="site-header__logo">
        <img src="img/Logo MM - branco.svg" alt="Logo Meinberg" />
      </div>

      <div class="site-header__menu-icon">
        <div class="site-header__menu-icon__middle"></div>
      </div>

      <div class="site-header__menu-content">
        <div class="site-header__btn-container">
          <a href="#" class="btn">Entre em contato</a>
        </div>
        <nav class="primary-nav primary-nav--col-right">
          <ul>
            <li><a href="#">Quem Somos</a></li>
            <li><a href="#">Seguros e Serviços</a></li>
            <li><a href="#">Fale Conosco</a></li>
          </ul>
        </nav>
      </div>
    </div>
  </header>

  <div class="large-hero">
    <picture>
      <source media="(min-width: 1380px)" srcset="img/hero--large.jpg" />
      <source media="(min-width: 990px)" srcset="img/hero--medium.jpg" />
      <source media="(min-width: 640px)" srcset="img/hero--small.jpg" />
      <img src="img/hero--smaller.jpg" class="large-hero__image" alt="Skyline da Cidade de São Paulo" />
    </picture>
    <div class="large-hero__text-content">
      <div class="titulo">
        <h1 class="large-hero__title">Meinberg</h1>
        <h2 class="large-hero__subtitle">Corretor de Seguros</h2>
        <p class="large-hero__decription">
          Corretagem de Seguros / Serviços Financeiros / Serviços Imobiliários
        </p>
      </div>
    </div>
  </div>

  <section class="servicos">
    <div class="wrapper">
      <h2 class="headline headline--centered">Nossos Serviços</h2>
      <h3
        class="headline headline--centered headline--small headline--gold headline--small headline--narrow headline--b-margin">
        Soluções para <strong>Você</strong> e sua <strong>Família</strong>,
        benefícios para sua <strong>Empresa</strong>.
      </h3>

      <div class="grid">
        <div class="cards">
          <h3 class="headline headline--centered headline--bold headline--smaller">
            <ion-icon name="heart-outline"></ion-icon><br>
            Seguro Vida
          </h3>
          <p>
            Os seguros de vida foram desenvolvidos para assegurar o futuro financeiro da sua família e de quem importa
            para você, sem atrapalhar sua renda atual. Os planos geralmente cobrem várias situações: acidentes pessoais,
            morte acidental, morte natural e assistências personalizadas. Conheça nossos produtos e entre em contato
            para escolher a melhor opção.
          </p>
          <a href="#" class="btn btn--small">Saiba Mais</a>
        </div>
        <div class="cards">
          <h3 class="headline headline--centered headline--bold headline--smaller">
            <ion-icon name="cash-outline"></ion-icon><br>
            Soluções para Locação
          </h3>
          <p>
            As soluções de locação foram desenvolvidas para facilitar a vida de proprietários de imóveis, imobiliárias e
            inquilinos, substituindo com vantagens o fiador. As soluções trazem tranquilidade ao proprietários para o
            recebimento do aluguel e encargos vencidos, em caso de não pagamento pelo inquilino. Entre em contato para
            saber mais.
          </p>
          <a href="#" class="btn btn--small">Saiba Mais</a>
        </div>
        <div class="cards">
          <h3 class="headline headline--centered headline--bold headline--smaller">
            <ion-icon name="construct-outline"></ion-icon><br>
            Riscos de Engenharia
          </h3>
          <p class="headline--red">
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Fugiat
            adipisci ea nobis eum perspiciatis quod officia, similique
            praesentium porro, eius voluptatibus vitae placeat qui inventore
            distinctio quidem repudiandae illo beatae.
          </p>
          <a href="#" class="btn btn--small">Saiba Mais</a>
        </div>
        <div class="cards">
          <h3 class="headline headline--centered headline--bold headline--smaller">
            <ion-icon name="home-outline"></ion-icon><br>
            Seguro Residência
          </h3>
          <p>
            Com um seguro residencial sua casa fica protegida por um custo muito menor do que você imagina. Além de
            proteger o bem mais precioso da sua família, você ainda conta com profissionais especializados em casos de
            pequenos imprevistos do dia a dia como reparos em eletrodomésticos, assistência de encanador e eletricista.
            Sem riscos e dores de cabeça, tudo por um custo que cabe no seu bolso. Fale conosco e faça sua cotação
          </p>
          <a href="#" class="btn btn--small">Saiba Mais</a>
        </div>
        <div class="cards">
          <h3 class="headline headline--centered headline--bold headline--smaller">
            <ion-icon name="business-outline"></ion-icon><br>
            Seguro Empresarial
          </h3>
          <p>
            O seguro empresarial é destinado para a pessoa física ou jurídica, proprietário ou locatário de um
            estabelecimento comercial, industrial ou de prestação de serviço. Ele garante que garantir o patrimônio da
            sua empresas está protegido contra os riscos a que estão expostos, proporcionando mais segurança e
            tranquilidade para sua você. Entre em contato e saiba todas as formas como sua empresa pode ficar protegida.
          </p>
          <a href="#" class="btn btn--small">Saiba Mais</a>
        </div>
        <div class="cards">
          <h3 class="headline headline--centered headline--bold headline--smaller">
            <ion-icon name="medkit-outline"></ion-icon><br>
            Seguro Saúde
          </h3>
          <p>
            Seguro saúde é um seguro que visa garantir uma melhor qualidade de vida e mais tranquilidade para você e sua
            família, oferecendo assistência médica, terapêutica, hospitalar e obstétrica com opções de profissionais e
            estabelecimentos de saúde. Os seguros saúde podem ser contratados em 3 modalidades: individual/familiar,
            coletivo por adesão ou coletivo empresarial. Entre em contato para saber qual a melhor opção para o seu
            caso.
          </p>
          <a href="#" class="btn btn--small">Saiba Mais</a>
        </div>
        <div class="cards">
          <h3 class="headline headline--centered headline--bold headline--smaller">
            <ion-icon name="car-outline"></ion-icon><br>
            Seguro Automóvel
          </h3>
          <p>
            O seguro de veículo (automóvel e motocicleta) é a forma mais completa de manter seu bem em segurança, mesmo
            quando você não está presente. Com diversas opções de seguros que combinam com o seu estilo de vida e suas
            necessidades temos as melhores soluções para o seu carro, moto, caminhão ou táxi. Escolha a alternativa que
            se encaixa melhor com a sua necessidade e fale conosco para ter uma cotação.
          </p>
          <a href="#" class="btn btn--small">Saiba Mais</a>
        </div>
        <div class="cards">
          <h3 class="headline headline--centered headline--bold headline--smaller">
            <ion-icon name="wallet-outline"></ion-icon><br>
            Previdência
          </h3>
          <p>
            A previdência privada é uma forma de você assegurar um futuro tranquilo contribuindo com uma pequena quantia
            em dinheiro por um determinado período. Esse esse valor fica rendendo, e quanto mais tempo o dinheiro ficar
            investido, mais ele vai render. Os pagamentos podem ser mensais ou de uma só vez e você ainda pode fazer
            contribuições adicionais sempre que tiver algum dinheiro sobrando. Você pode retirar seu dinheiro quando
            precisar. Entre em contato para saber mais.
          </p>
          <a href="#" class="btn btn--small">Saiba Mais</a>
        </div>
      </div>
    </div>
  </section>

  <section class="parceiras">
    <div class="wrapper">
      <h2 class="headline headline--centered headline--gold">
        <strong>Nossas Parceiras</strong>
      </h2>
      <h3
        class="headline headline--centered headline--small headline--gold headline--small headline--narrow headline--b-margin">
        As melhores seguradoras do mercado, você encontra em um lugar.
      </h3>
    </div>
    <div class="wrapper">
      <div class="galeria">
        <div class="logos">
          <img src="img/Seguradoras/allianz.jpg" alt="Logo Allianz" />
        </div>
        <div class="logos">
          <img src="img/Seguradoras/azul.jpg" alt="Logo Azul Seguros" />
        </div>
        <div class="logos">
          <img src="img/Seguradoras/bradesco.jpg" alt="Logo Bradesco Seguro" />
        </div>
        <div class="logos">
          <img src="img/Seguradoras/chubb.jpg" alt="Logo Chubb Seguros" />
        </div>
        <div class="logos">
          <img src="img/Seguradoras/hdi.jpg" alt="Logo HDI Seguros" />
        </div>
        <div class="logos">
          <img src="img/Seguradoras/humanas-seguros.jpg" alt="Logo Humanas Seguros" />
        </div>
        <div class="logos">
          <img src="img/Seguradoras/mag-seguros.jpg" alt="Logo MAG Seguros" />
        </div>
        <div class="logos">
          <img src="img/Seguradoras/mapfre.jpg" alt="Logo Mapfre Seguradora" />
        </div>
        <div class="logos">
          <img src="img/Seguradoras/porto.jpg" alt="Logo Porto Seguro" />
        </div>
        <div class="logos">
          <img src="img/Seguradoras/sulamerica.jpg" alt="Logo Sulamérica Seguros" />
        </div>
        <div class="logos">
          <img src="img/Seguradoras/univida.png" alt="Logo Univida Seguro de Pessoas" />
        </div>
        <div class="logos">
          <img src="img/Seguradoras/unisaude-master.png" alt="Logo Unisaúde Master" />
        </div>
      </div>
    </div>
  </section>

  <section class="fale-conosco" id="form">
    <div class="row">
      <h2 class="headline headline--centered">
        <strong>Fale Conosco</strong>
      </h2>
      <h3 class="headline headline--centered  headline--gold headline--small headline--narrow headline--b-margin">
        Ficaremos felizes com o seu contato. Peça uma cotação ou tire suas dúvidas.
      </h3>
    </div>

    <form method="POST" action="https://formspree.io/f/myybkpnv" class="contact-form">
      <div class="row">
        <div class="row">
          <div class="col-2-of-3">
            <input type="text" name="name" id="name" placeholder="Nome" required>
            <input type="tel" name="telefone" id="telefone" placeholder="Telefone" required>
            <input type="email" name="email" id="email" placeholder="E-mail" required>
            <input type="text" name="assunto" id="assunto" placeholder="Assunto" required>
            <textarea name="mgs" id="msg" cols="30" rows="10" placeholder="Deixe a sua mensagem" required></textarea>
            <input type="submit" value="Enviar">

          </div>
        </div>
        </div>
      </div>
    </form>
  </section>

  <footer class="site-footer">
    <div class="wrapper">
      <p>
        <span class="site-footer__text">Copyright &copy; 2020 <strong>Meinberg Corretora de Seguros</strong>. All rights
          reserved. Criado e desenvolvido por <strong><a href="https://fernandochiquito.github.io"
              target="_blank">Fernando Chiquito</a></strong>.
        </span>
        <a href="mailto:paulo@meinberg.biz" title="Entre em contato pra ter a sua cotação!" class="btn btn--small">Entre
          em Contato</a>
      </p>
    </div>

  </footer class="site-footer">
  <script type="module" src="js/app.js"></script>
  <script src="https://unpkg.com/ionicons@5.1.2/dist/ionicons.js"></script>
</body>

</html>